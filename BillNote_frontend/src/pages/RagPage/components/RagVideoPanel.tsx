...
const RagVideoPanel = () => {
  ...
  const ingestTaskId = useTaskStore(state => state.ingestTaskId)
  ...
  return (
    <>
      ...
      <div className="border-b border-slate-100 bg-slate-50 p-4">
        <div className="grid grid-cols-2 gap-1 rounded-lg bg-slate-200/50 p-1">
          ...
          <button
            ...
            className={[
              'flex items-center justify-center gap-1.5 rounded-md py-2 text-xs font-semibold transition-all',
              tab === 'batch' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700',
            ].join(' ')}
          >
            批量入库
          </button>
      ...
      <div className={cn('flex-1 overflow-y-auto')}>
        ...
        {tab === 'tasks' && (
          <div className="p-4">
            <NoteHistory onSelect={setIngestTask} selectedId={ingestTaskId} />
          </div>
        )}
      </div>
    </>
